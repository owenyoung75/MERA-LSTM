...
class DataSetS2S(object):
    def __init__(self,
                 ...
                 num_steps,
                 num_test_steps=None,
                 num_lags = None,           ## added 07/21/2018
                 seed=None):
        """Construct a DataSet.
        Seed arg provides for convenient deterministic testing.
        """
...
#        dec_inps = np.insert(data[:,num_steps:num_steps+num_test_steps-1,:], 0, SoS, axis=1)
        dec_inps = data[:,(num_steps-num_lags):(num_steps+num_test_steps-num_lags),:]
    ...
    def next_batch(self, batch_size, shuffle=True):
        ...
        if start + batch_size > self._num_examples:
            ...
            return self._enc_inps[start:end], \
                   self._dec_inps[start:end], \
                   self._dec_outs[start:end]



def read_data_sets(data_path, s2s,
                   n_steps,
                   n_test_steps = None,
                   n_lags = None,           ## added 07/21/2018
                   val_size = 0.1,
                   test_size = 0.1,
                   seed=None):
    ...
    train_options = dict(num_steps = n_steps,
                         num_test_steps = n_test_steps,
                         num_lags = n_lags,                 ## added 07/21/2018
                         seed = seed)
    if s2s == True:
